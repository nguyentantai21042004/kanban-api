...
func (h handler) processCreateRequest(c *gin.Context) (createReq, models.Scope, error) {
	...
	p, ok := scope.GetPayloadFromContext(ctx)
	if !ok {
		h.l.Errorf(ctx, "internal.boards.delivery.http.processCreateRequest.jwt.GetPayloadFromContext: %v", "payload not found")
		return createReq{}, models.Scope{}, pkgErrors.NewUnauthorizedHTTPError()
	}

	var req createReq
	if err := c.ShouldBindJSON(&req); err != nil {
		h.l.Errorf(ctx, "internal.boards.delivery.http.processCreateRequest.c.ShouldBindQuery: %v", err)
...
func (h handler) processUpdateRequest(c *gin.Context) (updateReq, models.Scope, error) {
	...
	p, ok := scope.GetPayloadFromContext(ctx)
	if !ok {
		h.l.Errorf(ctx, "internal.boards.delivery.http.processUpdateRequest.jwt.GetPayloadFromContext: %v", "payload not found")
		return updateReq{}, models.Scope{}, pkgErrors.NewUnauthorizedHTTPError()
	}

	var req updateReq
	if err := c.ShouldBindJSON(&req); err != nil {
		h.l.Errorf(ctx, "internal.boards.delivery.http.processUpdateRequest.c.ShouldBindQuery: %v", err)
...
func (h handler) processDetailRequest(c *gin.Context) (string, models.Scope, error) {
	...
	p, ok := scope.GetPayloadFromContext(ctx)
	if !ok {
		h.l.Errorf(ctx, "internal.boards.delivery.http.processDetailRequest.jwt.GetPayloadFromContext: %v", "payload not found")
		return "", models.Scope{}, pkgErrors.NewUnauthorizedHTTPError()
	}

	id := c.Param("id")
	if err := postgres.IsUUID(id); err != nil {
		h.l.Errorf(ctx, "internal.boards.delivery.http.processDetailRequest.c.Param: %v", err)
...
func (h handler) processDeleteRequest(c *gin.Context) (deleteReq, models.Scope, error) {
	...
	var req deleteReq
	if err := c.ShouldBindJSON(&req); err != nil {
		h.l.Errorf(ctx, "internal.boards.delivery.http.processDeleteRequest.c.ShouldBindQuery: %v", err)
